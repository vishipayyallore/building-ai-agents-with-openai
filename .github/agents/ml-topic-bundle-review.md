...
2. **Numbering:** Confirm layer-aware prefixes (`01`–`99`, never `00-` / `00_`); term map at `src/course-roadmap-and-module-overview.md`; topic index in the week's `01-introduction-*.md` note matches files (`.cursor/rules/08_file-naming-conventions.mdc`).
...
9. **Implementation notebooks:** Note if notebook Markdown introduces algorithms without concept-first prose.
