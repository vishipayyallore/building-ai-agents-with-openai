...
Batch-converts **PDF**, **HTML**, **PPTX**, and **Word (DOCX/DOC)** under `source-material/` to colocated `.md` staging files. Synthesize results into `docs/`, `sessions/session-NN-*/`, and `sessions/session-NN-*/speaker-notes.md` — see [tools/pyscripts/README.md](../pyscripts/README.md) (workflow + migration target map).
...
Cursor rule files (`.cursor/rules/*.mdc`) are IDE-local. Use these repository references for the canonical structure and CI checks:

- [Repository structure](../../docs/01-repository-structure.md) — canonical workspace layout
- [CLAUDE.md](../../CLAUDE.md) — governance index, CI commands, and key rule-file table
- [CI checks skill](../../.github/skills/ci-checks/SKILL.md) — local checks aligned with GitHub Actions
