...
# Rule: No 00- / 00_ prefix on files or folders
$scanRoots = @(
    (Join-Path $repoRootPath 'src')
    (Join-Path $repoRootPath 'docs')
    (Join-Path $repoRootPath 'presentation')
) | Where-Object { Test-Path -LiteralPath $_ }

$bad00 = foreach ($root in $scanRoots) {
    Get-ChildItem -Path $root -Recurse -Force -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -match '^00[-_]' }
}
if ($bad00) {
    $failed = $true
    Write-ComplianceError "Found disallowed '00-' or '00_' file/folder prefix (topic numbering starts at 01; use descriptive names for meta docs)."
