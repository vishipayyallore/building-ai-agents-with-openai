...
original content under ``docs/`` or ``presentation/demo-0N/`` before publishing
(Zero-Copy Policy).

Usage (PowerShell):
    uv run python tools/pyscripts/html_to_md.py \
        --input "source-material/session-01/slides/session-01-deck.html" \
        --output-same-folder

    uv run python tools/pyscripts/html_to_md.py --input source-material \
...
def _html_fragment_to_markdown(html_fragment: str) -> str:
    ...
    fragment = re.sub(
        r"</?(?:ul|ol|div|span|section|article|nav|button|strong|em|b|i)[^>]*>",
        "\n",
        fragment,
        flags=re.IGNORECASE,
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Convert HTML slide deck(s) to Markdown (raw extraction)."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Write each .md file in the same folder as its source .html (overrides --output-dir)."
        ),
    )
    parser.add_argument(
