...
Treat output as a staging artifact — synthesize into original content under
``docs/`` or ``presentation/demo-0N/`` before publishing (Zero-Copy Policy).

- ``.docx`` — extracted with ``python-docx``
- ``.doc``  — pandoc first; on Windows falls back to Word COM → docx when
    pandoc cannot read legacy DOC

Usage (PowerShell):
    uv run python tools/pyscripts/docx_to_md.py --input source-material \
        --recursive --output-same-folder

    uv run python tools/pyscripts/docx_to_md.py \
        --input "source-material\\universitynotes\\file.docx" --output-same-folder
"""
...
def docx_to_markdown(input_docx: Path, output_md: Path) -> None:
    ...
    lines.append(
        ...
        "Please **synthesize** into original content before publishing to `docs/` or "
        "`presentation/demo-0N/` to respect the repository's Zero-Copy Policy."
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Convert DOCX/DOC file(s) to Markdown (raw extraction)."
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Write each .md file in the same folder as its source Word file "
...
def main() -> int:
    ...
    output_dir = (
        args.output_dir if args.output_dir is not None else workspace_root / "docs" / "exports"
    )
    ...
    for word_file in word_files:
        out_md = (
            word_file.with_suffix(".md")
            if args.output_same_folder
            else output_dir / f"{word_file.stem}.md"
        )
        try:
