...
Swamy can then *synthesize* proper learning notes in `docs/` and
`presentation/demo-0N/` without violating the repo's Zero-Copy Policy.
...
Usage (PowerShell, from repo root):
    uv run python tools/pyscripts/pptx_to_md.py --input "C:\\path\\deck.pptx" \
        --extract-images --include-notes
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Convert PPTX slide(s) to Markdown (raw extraction)."
    )
...
def main() -> int:
    ...
    for input_pptx in pptx_files:
        ...
        if args.extract_images:
            actual_images_dir = (
                per_file_images_dir or output_md.parent / f"{input_pptx.stem}_images"
            )
