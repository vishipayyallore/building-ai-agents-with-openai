...
Usage (PowerShell):
  # Colocated .md next to each .pdf (recommended for PDFs under source-material):
    uv run python tools/pyscripts/pdf_to_md.py --input source-material \
        --recursive --output-same-folder

  # Image-only / scanned PDFs (requires Tesseract on PATH or standard Windows install):
    uv run python tools/pyscripts/pdf_to_md.py --input path/to/scanned.pdf \
        --output-same-folder --ocr

  # OCR only when pypdf returns no body text:
    uv run python tools/pyscripts/pdf_to_md.py --input source-material \
        --recursive --output-same-folder --ocr-when-empty
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Convert PDF(s) to Markdown (raw extraction, optional OCR)."
    )
