...
Requires: pip install openai-whisper (and ffmpeg on PATH for video files).
    uv run python tools/pyscripts/video_to_transcript.py \
...
Usage (PowerShell):
    ...
    uv run python tools/pyscripts/video_to_transcript.py \
        --input "source-material\\livesessions" --recursive
...
def transcribe_to_markdown(
    ...
    *,
    ...
    try:
        ...
        raise SystemExit(
            "OpenAI Whisper is required. Install with: uv add openai-whisper "
